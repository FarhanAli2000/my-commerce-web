...
const HealthCareComp = () => {
  ...
  useEffect(() => {
    ...
    filterCars(
      ...
      Type,
      MeasurementRange,
      Accuracy,CuffSize,DisplayType,BatteryType,Compatibility,StorageCapacity,MeasurementUnits,SpeedofMeasurement,SellerType,
    );
  }, [
    selectedCities,
    searchQuery,
    ...
    MeasurementRange,
    Accuracy,
    CuffSize,DisplayType,BatteryType,Compatibility,StorageCapacity,MeasurementUnits,SpeedofMeasurement,SellerType,
  ]);
  ...
  const filterCars = (
    ...
    if (SpeedofMeasurement?.length > 0) {
      filtered = filtered.filter((car) => SpeedofMeasurement.includes(car.SpeedofMeasurement));
    }
    if (MeasurementUnits?.length > 0) {
      filtered = filtered.filter((car) => MeasurementUnits.includes(car.MeasurementUnits));
    }
    if (StorageCapacity?.length > 0) {
      filtered = filtered.filter((car) => StorageCapacity.includes(car.StorageCapacity));
    }
    if (Compatibility?.length > 0) {
      filtered = filtered.filter((car) => Compatibility.includes(car.Compatibility));
    }
    if (BatteryType?.length > 0) {
      filtered = filtered.filter((car) => BatteryType.includes(car.BatteryType));
    }
    if (DisplayType?.length > 0) {
      filtered = filtered.filter((car) => DisplayType.includes(car.DisplayType));
    }
